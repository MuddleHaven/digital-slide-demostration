...
export default class OSDMeasure {
  ...
  constructor(viewer, options = {}) {
    ...
    this.viewer.addHandler('canvas-double-click', this.addMeasurementHandler.bind(this));
    ...
    // Add right-click event listener for showing custom context menu
    // this.viewer.addHandler('canvas-contextmenu', (event) => {
    //   // event position x, y is in measurement coordinates
  ...
  enableMeasurement() {
    ...
    this.enabled = true;
    // console.log('add measurement handler');
    // this.viewer.addHandler('canvas-double-click', this.addMeasurementHandler.bind(this));
  }
  ...
  disableMeasurement() {
    ...
    this.enabled = false;
    // console.log('remove measurement handler');
    // this.viewer.removeHandler('canvas-double-click', this.addMeasurementHandler.bind(this));
  }

  addMeasurementHandler(event) {
    if (this.enabled) {
   ...
   */
  exportCSV(sliceId) {
    const sId = sliceId || this.imageIdentifier;
    // let header = ["Name", "Point 1 X", "Point 1 Y", "Point 2 X", "Point 2 Y", "Distance(mm)"];
    let header = ["名称", "起点 X", "起点 Y", "终点 X", "终点 Y", "距离(微米)"];
    ...
    // 添加BOM标记，确保Excel能正确识别UTF-8编码的中文
    ...
    // 创建文件名: 切片id_年月日_时分秒.csv
    const filename = `测量数据_${sId}_${dateString}.csv`;
  ...
  /// Add a method to handle right-click events
  handleRightClick(event) {
    ...
    for (let i = 0; i < this.measurements.length; i++) {
      if (this.measurements[i].isPointInside(imagePoint.x, imagePoint.y)) {
        this.deleteMeasurement(this.measurements[i]);
        break;
      }
    }
  }

  /// Add a method to delete a measurement
  deleteMeasurement(measurement) {
    ...
    if (measurementIndex !== -1) {
      ...
      // Remove it from the database
      this.redoStack.push(measurement);
      this.db.removeMeasurement(measurement, this.imageIdentifier);
      this.saveInLocalStorage(this.imageIdentifier);
  ...
  showContextMenu(event) {
    ...
    contextMenu.style.boxShadow = '0px 0px 5px rgba(0,0,0,0.5)';

    // Add delete option to the context menu
    const deleteOption = document.createElement('div');
    deleteOption.innerText = 'Delete Measurement';
    ...
    // Add clear all option to the context menu
    const clearAllOption = document.createElement('div');
    clearAllOption.innerText = 'Clear All Measurements';
    ...
    const existingMenu = document.querySelector('.osd-context-menu');
    if (existingMenu) {
      document.body.removeChild(existingMenu);
    }

    // Add the context menu to the body
    contextMenu.classList.add('osd-context-menu');
...
class Measurement {
  ...
  adjustToZoom(zoom) {
    ...
    // 线条宽度调整，但设置最小宽度
    this.line.strokeWidth = Math.max(2, 50 / zoom);

    // 计算字体大小，但设置最小值和最大值
    // 最小值确保文字永远可读，最大值避免文字过大
    const calculatedFontSize = 300 / zoom;
    const minFontSize = 150; // 最小字体大小
    const maxFontSize = 300; // 最大字体大小

    this.textObject.fontSize = Math.min(maxFontSize, Math.max(minFontSize, calculatedFontSize));

    // 调整文本和点之间的距离，确保不会重叠
    const textPadding = Math.max(20, 100 / zoom);
    this.textObject.left = Math.max(this.p1.x, this.p2.x) + textPadding;

    // 调整文本位置，避免被线条遮挡
    const yOffset = (zoom > 5) ? -15 : 0; // 高缩放比例时稍微上移
    this.textObject.top = (this.p1.x > this.p2.x ? this.p1.y : this.p2.y) + yOffset;
  }
  ...
  render(zoom) {
    ...
    this.textObject = new fabric.FabricText(text, {
      left: Math.max(this.p1.x, this.p2.x) + 100 / zoom,
      top: this.p1.x > this.p2.x ? this.p1.y : this.p2.y,
      // fontSize: 300 / zoom,
      fontSize: Math.min(150, Math.max(100, 300 / zoom)),
      padding: 3,
      fill: this.color
    });
...
class MeasurementListItem {
  ...
  constructor(plugin, measurement) {
    ...
    // Add right-click event listener for deleting this measurement
    this.element.addEventListener("contextmenu", (event) => {
      event.preventDefault();
      this.plugin.deleteMeasurement(this.measurement);
    });
  }
